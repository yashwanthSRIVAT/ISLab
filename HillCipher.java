...
public class HillCipher {
    ...
    public boolean check(String key, int len) {
        ...
        d = d % 26;
        if (d == 0) {
            System.out.println("Invalid key!! Key is not invertible becasue determinant = 0...");
            return false;
        }
    ...
    public void confact(int num[][], int f) {
        int b[][], fac[][];
        b = new int[f][f];
        fac = new int[f][f];
        int p, q, m, n, i, j;
    ...
    public void matrixIntoInvKey( int inv[][], int n) {
        String invKey = "";
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) {
